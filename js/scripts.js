...
document.addEventListener('DOMContentLoaded', function () {
    const words = ['Creative', 'Amazing', 'Inovative', 'Everything!', 'Stunning'];
    let index = 0;

    function changeText() {
        document.getElementById('changingText').textContent = words[index];
        index = (index + 1) % words.length;
        setTimeout(changeText, 3000); // Change text every 1000 milliseconds (1 second)
    }

    // Initial call to start the dynamic text change
    changeText();
});
// curser
